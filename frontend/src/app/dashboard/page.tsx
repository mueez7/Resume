...
import { Upload, Sparkles, X, FileText, BarChart, Briefcase, Settings, LogOut, Search, Bell, AlertCircle, CheckCircle2, Eye, EyeOff, Loader2 } from "lucide-react";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    if (localStorage.getItem("aura_analysis")) setHasResults(true);
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      setProcessingStep("AI is analyzing your resume...");
      const data = await res.json();

      if (data.error) throw new Error(data.error);

      localStorage.setItem("aura_analysis", JSON.stringify(data));
      localStorage.setItem("aura_context", JSON.stringify({ targetRole: targetRole || "Software Engineer", salary, workType }));
      setHasResults(true);
  ...
  if (isProcessing) {
    return (
      <div className="h-screen bg-black flex flex-col items-center justify-center p-6 relative overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-acid/10 via-black to-black" />
        <div className="z-10 flex flex-col items-center text-center">
          <div className="relative w-32 h-32 mb-8">
            <div className="absolute inset-0 rounded-full border-t-2 border-acid animate-spin opacity-50" />
            <div className="absolute inset-2 rounded-full border-r-2 border-white animate-[spin_2s_linear_infinite] opacity-70" />
            <div className="absolute inset-4 rounded-full border-b-2 border-acid animate-[spin_3s_linear_infinite]" />
          ...
          <p className="text-gray-400 font-mono tracking-widest max-w-md">{processingStep}</p>
        </div>
      </div>
    );
  }
  ...
  const stats = [
    { label: "Avg ATS Score", value: hasResults ? JSON.parse(localStorage.getItem("aura_analysis") || "{}").atsScore || "---" : "---", color: hasResults ? "text-acid" : "text-gray-500" },
    { label: "Saved Jobs", value: "0", color: "text-gray-500" },
    { label: "Resumes Analyzed", value: hasResults ? "1" : "0", color: "text-white" },
  ];
    ...
    <div className="flex-1 w-full bg-black p-4 flex flex-col md:flex-row gap-4 font-sans text-white min-h-screen relative">
      ...
      <div className="flex-1 flex flex-col gap-4 w-full">
        ...
        <div className="flex-1 flex flex-col gap-4 pb-10 mt-4 md:mt-0 w-full">
          {error && (
            <div className="bg-red-950/60 border border-red-500/50 rounded-2xl p-5 flex items-start gap-4">
              ...
              <button onClick={() => setError(null)} className="text-red-500 hover:text-white transition-colors ml-2"><X size={16} /></button>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 h-full min-h-max">
              <div className="xl:col-span-2 flex flex-col gap-4">
                <div className="pt-4 pb-2 px-2">
                  <h1 className="text-4xl font-heading font-black mb-2">Command Center, <span className="text-acid">{userName.split(" ")[0]}.</span></h1>
                  <p className="text-gray-400 font-mono text-sm max-w-2xl">Aura systems nominal. Upload a PDF resume to run the neural extraction engine.</p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {stats.map((stat, i) => (
                    <div key={i} className="bg-[#050505] border border-[#222] rounded-[2rem] p-6 hover:border-acid transition-colors group flex flex-col justify-between min-h-[160px]">
                ...
                <div className="bg-[#050505] border border-[#222] rounded-[2rem] p-8 flex-1 flex flex-col min-h-[300px]">
                  <div className="flex justify-between items-center mb-6">
                    <h2 className="text-xl font-heading font-black uppercase flex items-center gap-2"><Upload className="text-acid" /> Quick Deploy</h2>
                  </div>
                  <button onClick={() => setActiveTab("upload")} className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-[#333] hover:border-acid rounded-[1.5rem] bg-black hover:bg-acid/5 p-8 text-center cursor-pointer transition-all group relative overflow-hidden">
                    ...
                    <p className="text-gray-500 font-mono text-sm max-w-sm">Navigate to the upload center to analyze your resume.</p>
                  </button>
                </div>
              </div>
              <div className="bg-[#050505] border border-[#222] rounded-[2rem] p-6 flex flex-col gap-6">
                ...
                {hasResults ? (
                  <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
                    ...
                    <p className="text-sm text-gray-500 font-mono mb-6">Your latest resume analysis is ready.</p>
                    <Link href="/results" className="w-full brutalist-button py-3 text-sm bg-white text-black hover:bg-acid text-center block rounded-xl">View Report</Link>
                  </div>
                ) : (
                  <div className="flex-1 flex flex-col items-center justify-center text-center p-4">
                    <AlertCircle size={48} className="text-gray-600 mb-4" />
                    <h3 className="font-bold uppercase text-lg mb-2 text-gray-500">No Data</h3>
                    <p className="text-sm text-gray-600 font-mono">Upload a resume to generate an analysis.</p>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col gap-4">
              <div className="bg-[#050505] border border-[#222] rounded-[2rem] p-8 flex-1 flex flex-col">
                <div className="flex justify-between items-center mb-6 border-b border-[#222] pb-6">
                  <div>
                    <h2 className="text-3xl font-heading font-black uppercase flex items-center gap-3 mb-2"><Upload className="text-acid" size={28} /> Upload Resume</h2>
                    <p className="text-gray-400 font-mono text-sm">Upload your PDF resume to run AI analysis and ATS scoring.</p>
                  </div>
                ...
                <label onDragOver={(e) => e.preventDefault()} onDrop={handleFileDrop}
                  ...
                  <p className="text-gray-500 font-mono text-base max-w-md">Click or drag and drop. Only PDF files with selectable text are supported.</p>
                </label>
              </div>
            </div>
          )}
            ...
            <div className="bg-[#050505] border border-[#222] rounded-[2rem] p-8 flex-1 flex flex-col">
              <div className="flex justify-between items-center border-b border-[#222] pb-6 mb-6">
                <div>
                  ...
                  <p className="text-gray-400 font-mono text-sm">Job matching powered by your resume analysis.</p>
                </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center text-center p-8 bg-black rounded-[1.5rem] border border-[#222]">
                ...
                <h3 className="text-2xl font-black uppercase font-heading mb-2">Coming Soon</h3>
                <p className="text-gray-500 font-mono max-w-md">Job board integration is in development. Upload and analyze your resume to unlock personalized job matching.</p>
              </div>
            </div>
          )}
          ...
          {activeTab === "settings" && (
            <div className="bg-[#050505] border border-[#222] rounded-[2rem] p-8 flex-1 flex flex-col">
              <div className="border-b border-[#222] pb-6 mb-6">
                <h2 className="text-3xl font-heading font-black uppercase mb-2">Settings</h2>
                <p className="text-gray-400 font-mono text-sm">Account and preferences.</p>
              </div>
                  ...
                  <div className="space-y-4">
                    <div className="bg-black p-4 rounded-xl border border-[#222] flex justify-between items-center">
                      <div>
                        ...
                        <div className="text-gray-500 font-mono text-xs">{userName}</div>
                      </div>
                    </div>
                    <div className="bg-black p-4 rounded-xl border border-[#222] flex justify-between items-center">
                      <div>
                        <div className="font-bold text-sm">Email</div>
                        <div className="text-gray-500 font-mono text-xs">{user?.email || "Not logged in"}</div>
                      </div>
                    </div>
                  </div>
                </div>
                <div>
                  <h3 className="text-lg font-bold uppercase tracking-widest text-white mb-4">Actions</h3>
                  <div className="space-y-4">
                    <button onClick={handleLogout} className="bg-red-500/10 border border-red-500/30 hover:border-red-500 text-red-400 px-6 py-3 rounded-xl font-bold uppercase text-sm tracking-widest transition-all">Log Out</button>
                  </div>
                </div>
              </div>
            </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[100] flex items-center justify-center p-4">
          <div className="bg-[#050505] rounded-[2rem] border border-[#222] p-8 w-full max-w-lg relative">
            <button onClick={() => setShowModal(false)} className="absolute top-6 right-6 text-gray-500 hover:text-white w-8 h-8 flex items-center justify-center rounded-full hover:bg-[#111] transition-colors"><X size={20} /></button>
            <h3 className="text-2xl font-black uppercase mb-2 font-heading">Target Parameters</h3>
            <p className="text-gray-400 mb-6 text-sm font-mono border-b border-[#222] pb-6">
              File: <span className="text-acid">{file?.name}</span> ({((file?.size || 0) / 1024).toFixed(0)} KB)
            </p>
            <div className="space-y-6 mb-8">
              <div>
                ...
                <input type="text" value={targetRole} onChange={(e) => setTargetRole(e.target.value)}
                  placeholder="e.g. Senior Frontend Engineer" className="w-full bg-black border border-[#222] rounded-xl p-4 text-white focus:outline-none focus:border-acid font-mono transition-colors" />
              </div>
              <div>
                ...
                <input type="text" value={salary} onChange={(e) => setSalary(e.target.value)}
                  placeholder="e.g. $150,000" className="w-full bg-black border border-[#222] rounded-xl p-4 text-white focus:outline-none focus:border-acid font-mono transition-colors" />
              </div>
              <div>
            ...
            <button onClick={handleProcess} disabled={!targetRole}
